...
def initialize():
    ...
    day_count = []
    for x in range(len(trans_file)):
        day_count.append(x)
    trans_file['Day_Count'] = day_count
    print(trans_file)
    return trans_file, country_names

def create_lin_reg_plot(X, y, X_test, y_pred, country_name, days, intercept, coef):
    plt.scatter(X, y, color='gray')
    plt.plot(X_test, y_pred, color = 'red', linewidth=2)
    plt.title('Daily Covid-19 Cases in ' + country_name + ' Across ' + days + ' Days')
...
def create_linear_regression(df, country_name):
    ...
    create_lin_reg_plot(X, y, X_test, y_pred, country_name, str(len(df)), regressor.intercept_, regressor.coef_)
